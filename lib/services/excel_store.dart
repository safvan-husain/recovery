...
class ExcelStore {
  static Future<void> processExcelInStream() async {
    var uuid = const Uuid();
    var bytes = await rootBundle.load("assets/icons/large.xlsx");
    var excelFile =
        File("${(await getTemporaryDirectory()).path}/${uuid.v1()}");
    await excelFile.writeAsBytes(
        bytes.buffer.asUint8List(bytes.offsetInBytes, bytes.lengthInBytes));
    var input = excelFile.openRead();

    Stream<List<int>> inputStream = input;

    // var excel = Excel.decodeBytes(List.empty(growable: true));

    await for (var chunk in inputStream) {
      var excel = Excel.decodeBytes(chunk);
      print(excel.sheets.entries.map((e) => e.key));
      // Process the chunk here (e.g., extract data, perform operations)
    }
    print("completed processExcelInStream");
  }

  static Future<void> processExcelInChunks() async {
    // var file = 'path/to/your/excel/file.xlsx';
    var excel = Excel.createExcel(); // Create an empty Excel instance

    var bytes = await rootBundle.load("assets/icons/large.xlsx");

    var data = bytes.buffer.asUint8List();
    var len = data.length;
    var chunkSize = 1024 * 1024; // Define your chunk size, e.g., 1MB

    for (var i = 0; i < len; i += chunkSize) {
      var end = (i + chunkSize < len) ? i + chunkSize : len;
      var chunk = data.sublist(i, end);
      // excel.updateCell(
      //     'Sheet1', CellIndex.indexByString('A1'), TextCellValue('Google'));
      // await Future.delayed(
      //     Duration(milliseconds: 500)); // Simulate processing delay
      var tmpExcel = Excel.decodeBytes(chunk);
      print(tmpExcel.sheets.entries.map((e) => e.key));
      // Process tmpExcel (e.g., copy data to your main excel instance or perform operations)
    }
    print("processExcelInChunks completed");
  }

  static void downloadAndConvertExcelToCSV(String excelUrl) async {
    print("calling downloadAndConvertExcelToCSV");
    try {
      Dio dio = Dio();
      // Download Excel file
      print("before download");
      Response<List<int>> response = await dio.get<List<int>>(excelUrl,
          options: Options(responseType: ResponseType.bytes));
      print("after download");

      // Create an Excel instance from the downloaded file
      Excel excel = Excel.decodeBytes(response.data!);
      ...
      // Convert Excel table to CSV format
      // final csvData = table.;

      // Save CSV data to a file
      // String csvFileName =
      //     'converted_${DateTime.now().millisecondsSinceEpoch}.csv';
      // File csvFile = File(csvFileName);
      // await csvFile.writeAsString(csvData);

      // print('File converted and saved as: $csvFileName');
    } catch (e) {
      print('Error: $e');
    }
  }
